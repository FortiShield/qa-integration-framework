...
from fortishield_testing.constants.api import FORTISHIELD_API_PROTOCOL, FORTISHIELD_API_HOST, FORTISHIELD_API_PORT, FORTISHIELD_API_USER, \
                                        FORTISHIELD_API_PASSWORD, LOGIN_ROUTE, USERS_ROUTE, RESOURCE_ROUTE_MAP, \
                                        TARGET_ROUTE_MAP
from fortishield_testing.constants.paths.api import FORTISHIELD_API_CERTIFICATE
from fortishield_testing.modules.api.patterns import API_LOGIN_ERROR_MSG
from fortishield_testing.utils.file import read_json_file
...
def get_base_url(protocol: str = FORTISHIELD_API_PROTOCOL, host: str = FORTISHIELD_API_HOST, port: str = FORTISHIELD_API_PORT) -> str:
    """Get FortiShield API's base URL.
    ...
    Returns:
        base_url (str): FortiShield API's base URL
    """
...
def set_authorization_header(user: str = None, password: str = None) -> dict:
    ...
    """
    user = FORTISHIELD_API_USER if user is None else user
    password = FORTISHIELD_API_PASSWORD if password is None else password
...
def login(user: str = FORTISHIELD_API_USER, password: str = FORTISHIELD_API_PASSWORD,
          timeout: int = session_parameters.default_timeout, login_attempts: int = 3, backoff_factor: float = 0.5,
          host: str = FORTISHIELD_API_HOST, port: str = FORTISHIELD_API_PORT, protocol: str = FORTISHIELD_API_PROTOCOL
    ...
    response = session.post(url, headers=set_authorization_header(user, password), verify=FORTISHIELD_API_CERTIFICATE,
                                timeout=timeout)
...
def allow_user_to_authenticate(user_id: str = None) -> requests.Response:
    ...
    response = requests.put(url + params, headers=authentication_headers, verify=FORTISHIELD_API_CERTIFICATE)
...
def manage_security_resources(method: str = 'get', resource: Union[dict, str] = None,
    ...
    authentication_headers = login()[0]
    response = getattr(requests, method)(url, headers=authentication_headers, verify=FORTISHIELD_API_CERTIFICATE, json=payload)
...
def relate_resources(test_metadata: dict) -> None:
    ...
    for origin in relationships:
        ...
        for idx, target_value in enumerate(target_values):
            ...
            response = requests.post(url, headers=login()[0], verify=FORTISHIELD_API_CERTIFICATE)
            if response.status_code != 200 or response.json()['error'] != 0:
...
def remove_resources_relationship(origin_resource: dict = None, target_resource: dict = None) -> dict:
    ...
    response = requests.delete(url, headers=login()[0], verify=FORTISHIELD_API_CERTIFICATE)
    if response.status_code != 200 or response.json()['error'] != 0:
...
def get_manager_configuration(section=None, field=None):
    ...
    References: https://documentation.fortishield.com/current/user-manual/api/reference.html#operation/
...
def get_api_details_dict(protocol=FORTISHIELD_API_PROTOCOL, host=FORTISHIELD_API_HOST, port=FORTISHIELD_API_PORT, user=FORTISHIELD_API_USER, password=FORTISHIELD_API_PASSWORD,
