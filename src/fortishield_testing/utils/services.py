...
from fortishield_testing.constants.daemons import CLUSTER_DAEMON, API_DAEMON, FORTISHIELD_AGENT, FORTISHIELD_MANAGER, FORTISHIELD_AGENT_WIN
from fortishield_testing.constants.paths.binaries import BIN_PATH, FORTISHIELD_CONTROL_PATH
from fortishield_testing.constants.paths.sockets import FORTISHIELD_SOCKETS, FORTISHIELD_OPTIONAL_SOCKETS
from fortishield_testing.constants.paths.variables import VAR_RUN_PATH, VERSION_FILE
from fortishield_testing.constants.platforms import MACOS, SOLARIS, WINDOWS
...
def get_service() -> str:
    """
    Retrieves the name of the FortiShield service running on the current platform.

    Returns:
        str: The name of the FortiShield service. It can be either 'fortishield-manager'
             or 'fortishield-agent'.
    ...
    if platform.system() in ['Windows', WINDOWS]:
        return FORTISHIELD_AGENT

    else:  # Linux, sunos5, darwin, aix...
        service = subprocess.check_output([
            FORTISHIELD_CONTROL_PATH, "info", "-t"
        ], stderr=subprocess.PIPE).decode('utf-8').strip()

    return FORTISHIELD_MANAGER if service == 'server' else FORTISHIELD_AGENT


def get_version() -> str:
    """
    Retrieves the version of the FortiShield software installed on the current platform.

    Returns:
        str: The version of the FortiShield software.
    ...
    if platform.system() in ['Windows', WINDOWS]:
        ...
        return subprocess.check_output([
            FORTISHIELD_CONTROL_PATH, "info", "-v"
        ...
        if action == 'restart':
            ...
            for _ in range(error_109_windows_retry):
                command = subprocess.run(
                    ["net", action, "FortiShieldSvc"], stderr=subprocess.PIPE)
                result = command.returncode
                if result != 0:
                    if action == 'stop' and 'The FortiShield service is not started.' in command.stderr.decode():
                        result = 0
                        break
        ...
        if daemon is None:
            if sys.platform == MACOS or sys.platform == SOLARIS:
                result = subprocess.run(
                    [FORTISHIELD_CONTROL_PATH, action]).returncode
            else:
                result = subprocess.run(
            ...
            if action == 'restart':
                ...
                sockets.delete_sockets(FORTISHIELD_SOCKETS[daemon])
            else:
...
def check_all_daemon_status():
    ...
    if sys.platform == WINDOWS:
        current_daemon = 'FORTISHIELD_AGENT_WIN'
        daemons_status[current_daemon] = check_if_process_is_running(FORTISHIELD_AGENT_WIN)
    else:
        control_status_output = subprocess.run([FORTISHIELD_CONTROL_PATH, 'status'],
...
def wait_expected_daemon_status(target_daemon=None, running_condition=True, timeout=10, extra_sockets=[]):
    ...
    while elapsed_time < timeout and not condition_met:
        if sys.platform == WINDOWS:
            condition_met = check_if_process_is_running(FORTISHIELD_AGENT_WIN) == running_condition
        else:
            control_status_output = subprocess.run([FORTISHIELD_CONTROL_PATH, 'status'],
                                                   stdout=subprocess.PIPE).stdout.decode()
            condition_met = True
