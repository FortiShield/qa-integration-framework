...
from fortishield_testing.constants.paths.sockets import QUEUE_SOCKETS_PATH, FORTISHIELD_DB_SOCKET_PATH, \
                                                  MODULESD_C_INTERNAL_SOCKET_PATH, \
                                                  ACTIVE_RESPONSE_SOCKET_PATH
from fortishield_testing.utils.network import UDP
...
def delete_sockets(path=None):
    ...
    try:
        if path is None:
            ...
            if os.path.exists(FORTISHIELD_DB_SOCKET_PATH):
                os.remove(FORTISHIELD_DB_SOCKET_PATH)
...
def send_request_socket(query, socket_path=FORTISHIELD_DB_SOCKET_PATH):
