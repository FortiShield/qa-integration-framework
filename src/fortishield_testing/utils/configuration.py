...
from fortishield_testing.constants.paths.configurations import FORTISHIELD_CONF_PATH, FORTISHIELD_LOCAL_INTERNAL_OPTIONS
...
def get_minimal_configuration():
    ...
    Returns:
        List of str: FortiShield minimal configuration data.
    """
...
def get_fortishield_conf() -> List[str]:
    ...
    """
    return file.read_file_lines(FORTISHIELD_CONF_PATH)


def write_fortishield_conf(fortishield_conf: List[str]) -> None:
    """
    ...
    """
    file.write_file(FORTISHIELD_CONF_PATH, fortishield_conf)


def set_section_fortishield_conf(sections: List[dict], template: List[str] = None) -> List[str]:
    """
    ...
    for section in sections:
        ...
        section_conf = find_module_config(
            fortishield_conf, section['section'], attributes)
        # Create section if it does not exist, clean otherwise
        if not section_conf:
            section_conf = ET.SubElement(
                fortishield_conf.getroot(), section['section'])
            section_conf.text = '\n    '
            section_conf.tail = '\n\n  '
        else:
