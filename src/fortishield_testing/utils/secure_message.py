...
def get_agent_id(message: bytes) -> Union[str, None]:
    """
    Extracts the agent ID from a given FortiShield message.

    In FortiShield, the agent ID is sent within the message, enclosed between
...
def get_payload(message: bytes, algorithm: str) -> bytes:
    """
    Extracts the payload from a given FortiShield message.
...
def set_algorithm_header(message: bytes, algorithm: str) -> bytes:
    """
    Adds an encryption algorithm header to a given FortiShield message.
